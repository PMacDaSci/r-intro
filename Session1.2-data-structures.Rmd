---
title: "Introduction to Solving Biological Problems Using R - Week 1"
...
output:
  html_notebook:
    ...
    toc_float: yes
---
*Mark Dunning, Suraj Menon and Aiora Zabala. Original material by Robert Stojnić,
...
- In this example, we will use the gene.expression vector we created earlier.

```{r}
gene.expression
```

So far we have seen how to extract the first and third values in a vector

```{r}
gene.expression[c(1,3)]
```

R can perform the same operation using a vector of logical values. Only indices with a `TRUE` value will get returned

```{r}
gene.expression[c(TRUE, FALSE, TRUE, FALSE)]
```
...
```{r}
gene.expression[gene.expression == 0]
```

```{r}
gene.expression[gene.expression > 1 & gene.expression < 3]
```
...
* Although the basic unit of R is a vector, we usually handle data in data frames
* Each column of a data frame is a vector and can be either numeric, character or logical
* Columns from the data frame can be accessed using the ‘$’ operator
* A factor is R’s data structure for categorical data
* We can ask R not to treat strings as factors with `stringsAsFactors=FALSE`
* Data frames can be indexed using dataframe[rows, columns]
* Multiple logical tests can be combined with operators (e.g. dataframe[x > 1 & x < 3, ])